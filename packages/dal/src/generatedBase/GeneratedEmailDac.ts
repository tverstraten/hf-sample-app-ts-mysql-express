...
export class GeneratedEmailDac extends AbstractMutableDac<Email> 
{
	...
	protected fromRow(row: any): Email {
		...
		result.ccd = row['ccd'] == null ? undefined : row['ccd'] // string
		result.bccd = row['bccd'] == null ? undefined : row['bccd'] // string
		result.firstRead = row['firstRead'] == null ? undefined : new Date(row['firstRead']) // dateTime
