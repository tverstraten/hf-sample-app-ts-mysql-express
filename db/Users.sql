...
NOTE: access may also be restricted if the account is not currently enabled.',
	`lastInvitationSentOn` TIMESTAMP NULL COMMENT 'The last date an invitation was sent to this user. Previously invitations are not recorded.',
	`invitationAcceptedOn` TIMESTAMP NULL COMMENT 'The date an invitation was accepted by this user which would be the date they created an account.',
	PRIMARY KEY (`id`)
